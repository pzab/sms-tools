...
def minFreqEstErr(inputFile, f):
    ...
    i_05sec = np.argmin(abs(time-0.5))
    fEst = 10.
    M = 1099
    while abs(f-fEst)>0.05:
        M +=2
        N = int(pow(2, np.ceil(np.log2(M))))        
 
        x_center = x[int(i_05sec-M//2): int(i_05sec+M//2) + 1]

        w = get_window(window,M)
        mX,pX = DFT.dftAnal(x_center,w, N)
        ploc = UF.peakDetection(mX,t)[0]
